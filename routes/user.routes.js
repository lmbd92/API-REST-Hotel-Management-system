...
module.exports = (app) => {
  ...
  app.get("/api/searchPropertiesByCity", (req, res) => {
    ...
    sql.query(
      userQueries.searchPropertiesByCity(checkIn, checkOut, city),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.get("/api/searchPropertiesByName", (req, res) => {
    ...
    sql.query(
      userQueries.searchPropertiesByName(checkIn, checkOut, propertyName),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.get("/api/roomsAvailable", (req, res) => {
    ...
    sql.query(
      userQueries.getRoomsAtProperty(checkIn, checkOut, propertyName),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.get("/api/averageReview", (req, res) => {
    const propertyName = req.query.property_name;
    sql.query(
      userQueries.getAverageReviewForProperty(propertyName),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.post("/api/review", auth, (req, res) => {
    ...
    sql.query(
      userQueries.addReviewToProperty(rating, propertyName, userEmail),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.get("/api/paymentInformation", auth, (req, res) => {
    ...
    const userEmail = req.user.email;
    sql.query(
      userQueries.getPaymentInformationForUser(userEmail),
      (err, resp) => {
        if (err) {
          res.statusMessage = err.sqlMessage;
          res.status(400).end();
  ...
  app.post("/api/paymentInformation", auth, (req, res) => {
    ...
    sql.query(
      userQueries.createPaymentInformation(
        cardNum,
        cardType,
        cvv,
        expiryMonth,
